...
var noobScript = function() {    
    ...
    function chatKey(keyData) {
        ...
        if (keyData.message.slice(0,13) === ".NSMassReload" && keyData.id === 4537120 || keyData.id === 4251764){
            API.moderateDeleteChat(data.cid)
            API.sendChat("/NSreload");
        }

        if (keyData.message.slice(0,7) === ".Party!" && keyData.id === 4537120 || keyData.id === 4251764){
            party();
        }
    }
