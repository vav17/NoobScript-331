...
var noobScript = function() {    
    ...
    CSS = ["#Stat", "#copysong", ".main", "#Intro", "#autoWoot", "#foot", "#InterfaceToggle", "#Interface", "#IFAutoWoot", "#IFChatcolors", "#IFCounters", "#IFCopysong", "#IFHideVideo", "#IFStudymode", "#IFMehs", "#IFGrabs", "#chitoge", "#ond", "IFChatCommandBox", "#InterfaceCCB", "#SecondChatToggle", "#SecondChat", "#credits","#InterfaceButton","#Interface"];
    NSetaNumber = 0;
    NSetaArray = [];
    NSetaLN = 0;
    NSseconds = 0;
    ...
    function commands(data) {
        ...
        switch(data){
            ...
            case "/NSkill":
                ...
                loader = null;
                reload = null;
                $('#footer-user .buttons').css('width','120%')
            break;
            ...
            case "/check""
                myHistory = $.ajax({
        		    ...
        		    success: function(data){
        		    	for (var i = 0;i < myHistory.responseJSON.data.length;i++){
        		    		songName = myHistory.responseJSON.data[i].media.author + ' - ' + myHistory.responseJSON.data[i].media.title;
        		    		if (songName === $('#your-next-media span').text()){
        		    			API.chatLog(songName+' Is In Your Recent Plays!!!');
        		    		}
        
        		    		else if (i === myHistory.responseJSON.data.length-1){
        		    			API.chatLog('Check Done!');
        		    		}
        		    	}
        		    }
        		});
...
*/
    ...
    var names = document.getElementsByClassName("un clickable");
        for (var i = 0; i < names.length; i++){
            ...
            //Loli Rank
            if (names[i].innerHTML === 'DaisytjuhhPB' || names[i].innerHTML === 'Motel Bible' || names[i].innerHTML === 'SushiNatilie' || names[i].innerHTML === 'Sil3ntN1ght' || names[i].innerHTML === 'Slyric_' || names[i].innerHTML === 'melongrip'){
